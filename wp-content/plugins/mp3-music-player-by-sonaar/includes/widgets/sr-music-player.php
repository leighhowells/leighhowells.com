...
class SR_Audio_Player extends Widget_Base {
	...
	public function get_script_depends() {
		if (function_exists( 'run_sonaar_music_pro' ) && (\Elementor\Plugin::$instance->editor->is_edit_mode() || \Elementor\Plugin::$instance->preview->is_preview_mode())) {
			return ['elementor-sonaar', 'color-thief'];
		}else{
			return [ 'elementor-sonaar' ];
		}
	}
			...
			$this->add_control(
				...
				[
					...
					'description' => sprintf(
						...
						'<a href="' . esc_url(SRMP3_DIR_PATH . 'templates/example_of_csv_file_to_import.csv' ) . '" target="_blank">' . esc_html__('here', 'sonaar-music') . '</a>'
					),
...
// Deprecated control: play_current_id. It's always hidden except for old installation. This has been replaced by playlist_source = from_current_post
		...
		if ( function_exists( 'run_sonaar_music_pro' ) ){
			...
			$this->add_control(
				'spectro_notice',
				[
					'raw' => '<strong>' . __( 'Audio Not Playing? Spectrum Not Showing?', 'elementor' ) . '</strong> ' . __( 'If you don\'t see the Live Spectrum or Audio does not start when using this feature, <a href="https://bit.ly/3HtbC9u" target="_blank">Click here</a> for possible cause and work-around.', 'elementor' ),
			...
			$this->start_controls_section(
	            'player',
	            [
	                'label'							=> esc_html__( 'Mini Player & Progress Bar', 'sonaar-music' ),
					'tab'							=> Controls_Manager::TAB_STYLE,
					'conditions'                    => [
						'relation' => 'or',
						'terms' => [
				...
				$this->add_control(
					...
					[
						...
						'options' 				=> [
							...
							'mediaElement' 		=> esc_html__( 'Synthetic Waveform', 'sonaar-music' ),
						],
						'default' 				=> 'default',
						'conditions'                    => [
							'relation' => 'or',
			...
			//if(Sonaar_Music::get_option('waveformType', 'srmp3_settings_general') === 'simplebar'){
				$this->add_responsive_control(
					...
					[
						...
						'selectors' 					=> [
							'{{WRAPPER}} .wave, {{WRAPPER}} .sonaar_fake_wave, {{WRAPPER}} .sr_waveform_simplebar .sonaar_fake_wave .sonaar_wave_base, {{WRAPPER}} .sr_waveform_simplebar .sonaar_fake_wave .sonaar_wave_cut' => 'height: {{SIZE}}px !important;',
							'{{WRAPPER}} .sonaar_fake_wave' => 'margin-top: initial;margin-bottom:initial;max-height:unset',
							'{{WRAPPER}} .sr_progressbar' => 'margin-top: 20px;margin-bottom:20px',
						],
						'conditions'                    => [
							'relation' => 'or',
							'terms' => [
			...
			$this->add_control(
				...
				[
					...
					'return_value' 					=> 'yes',
					'selectors'            			=> [
						'{{WRAPPER}} .iron-audioplayer .srp_main_control .control .previous.sricon-back, {{WRAPPER}} .iron-audioplayer .srp_main_control .control .play, {{WRAPPER}} .iron-audioplayer .srp_main_control .control .next.sricon-forward' => 'display:none!important;',
					],
					'condition' 					=> [
						'player_layout!' 	=> 'skin_button',
					],
		...
		if (get_site_option('SRMP3_ecommerce') != '1'){
			...
			$this->add_control(
				'force_cta_dl',
				[
					'label'							=> sprintf( esc_html__( 'Display Download Buttons', 'sonaar-music' ), ucfirst(Sonaar_Music_Admin::sr_GetString('track')) ),
					'type' 							=> Controls_Manager::SELECT,
					'options' 		=> [
			...
			$this->add_control(
				...
				[
					'label'							=> sprintf( esc_html__( 'Display Link to Post Buttons', 'sonaar-music' ), ucfirst(Sonaar_Music_Admin::sr_GetString('track')) ),
					'type' 							=> Controls_Manager::SELECT,
					'options' 		=> [
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors' 					=> [
													'{{WRAPPER}} .iron-audioplayer .song-store-list-container .song-store .fab, {{WRAPPER}} .iron-audioplayer .song-store-list-container .song-store .fas' => 'font-size: {{SIZE}}px;',
					],
				]
			);
			$this->add_responsive_control(
